...
function fillEditExercise(homePage,id){
    ...
    var toolbarButton = document.getElementById("deleteIconEdit");
    toolbarButton.onclick = function(){
        if (confirm("Вы действительно хртите удалить?") == false) {
            return;
        }
        if(ids[0]=="LE"){
...
function validateForm(form,type){
    ...
    if(form[0].value.length>20){
        alert("Наименование должно содержать не более 20 символов!");
        return false;
    }
    if(type=="meal"){
        if((form[1].value==null||form[1].value==""||form[1].value==0)&&
            ...
            alert("Один из пунктов должен быть заполнен");
            return false;
        }
        ...
        if(!isOnlyNumber(form[1].value)){
            alert("Поле Белки должно содержать только цифры!");
            return false;
        }
        if(!isOnlyNumber(form[2].value)){
            alert("Содержит только цифры!");
            return false;
        }
        ...
        if(form[1].value==null||form[1].value==""){
            alert("Калории должны быть заполнены");
            return false;
        }
...
function fillEditRestriction(homePage,date){
    ...
    var toolbarButton = document.getElementById("deleteIconEdit");
    toolbarButton.onclick = function(){
        if (confirm("Вы действительно хотите сбросить ограничения?") == false) {
            return;
        }
...
function validateFormRestriction(form){
    ...
    if(!isOnlyNumber(form[0].value)&&form[0].value!=null&&form[0].value!=""){
        alert("Содержит только цифры или пустое поле");
        return false;
    }
