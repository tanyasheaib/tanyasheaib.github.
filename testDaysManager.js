...
QUnit.test( "добавить день", function( assert ) {
    assert.ok(globalDaysManager.isEmpty(),"Пусто");
    globalDaysManager.addDay(new Day(new Date()));
    globalDaysManager.addDay(new Day(new Date("2015-03-25")));
    assert.equal(2,globalDaysManager.getAllDays().length,"2 дня");
    ...
    assert.deepEqual(new Date("2015-03-25").toDateString(),date,"данные не одинаковы");
});
QUnit.test( "Добавление с неправильной датой", function( assert ) {
    globalDaysManager.addDay(new Day(new Date()));
    assert.throws(function() {globalDaysManager.addDay(new Date());},"Добавлено");
});
QUnit.test( "Добавление продукта в день", function( assert ) {
    globalDaysManager.addDay(new Day(new Date()));
...
QUnit.test( "test add exercise to day", function( assert ) {
    ...
    var day = globalDaysManager.getDayByDate(new Date());
    assert.notOk(day.exercisesManager.isEmpty(),"Упражнения не пусты");
